...
- Google subscription execution must use Google's official Antigravity CLI in headless/JSON mode. A package-managed provider binary must be installed in a private per-user data directory that is not a normal command directory, must use the provider installer's no-PATH/no-alias options, and must be invoked by absolute path so it never shadows this package's `agyc` command.
...
- Google authentication may open the provider's required browser sign-in page, but `agyc login` must not launch the provider's terminal TUI, folder-trust screen, IDE onboarding, banners, tips, or slash-command interface. A valid official Antigravity keyring session should be reused silently.
