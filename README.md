...
If both commands print versions and Node is a supported current release — **v20.11+**, **v22.x**, or **v24.x** — continue.

If `node` or `npm` says it is not recognized or not found, install a supported Node.js release, close the terminal, open a new terminal, and run the two commands again.
...
npm init -y
npm install C:\path\to\antigravity-cli-npm\antigyc-0.1.3.tgz
npx antigyc --version
npx antigyc --help
npx antigyc
```
...
Google subscription mode now executes through Google's **official Antigravity CLI headless client**, rather than the older Gemini CLI / Code Assist client that rejects personal accounts. The provider binary is invoked by its absolute per-user path. Requests are sent through the provider's supported `stream-json` stdin transport instead of being embedded in a `-p` process argument, which avoids Windows command-line-length failures during long conversations. Stream events and final JSON results are captured internally, so the provider TUI, progress stream, slash commands, banners, and tool narration do not appear inside this wrapper.
...
A fresh profile starts with turbo off and `approval ask`. This keeps autonomous coding available while requiring an explicit opt-in before unrestricted command execution. Enable turbo when you want no-prompt autonomous command execution:
...
src/command-sandbox.js  OS-level sandbox policy for direct API command execution
src/command-child.js    structured command trampoline used inside the sandbox
src/environment.js      filtered subprocess environment handling
src/process.js          cross-platform subprocess-group/tree termination
...
This repository is currently configured with version `0.1.3` and `publishConfig.access = public`. Publication is a separate explicit step; running the commands above does not publish anything.
